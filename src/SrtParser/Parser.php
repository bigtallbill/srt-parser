...
class Parser
{
    ...
    private static function splitData($data)
    {
        ...
        $sections = preg_split('/\d+(?:\r\n|\r|\n)(?=(?:\d{1,2}:\d{1,2}:\d{1,2}[,.]\d{3})\s-->\s(?:\d{1,2}:\d{1,2}:\d{1,2}[,.]\d{3}))/m', $data,-1,PREG_SPLIT_NO_EMPTY);
        $matches = [];
    ...
    {
        $matches = [];
        preg_match_all('/(\d{1,2}:\d{2}:\d{2}[,.]\d{3})\s-->\s(\d{1,2}:\d{2}:\d{2}[,.]\d{3})/mi', $timeString, $matches,
            PREG_SET_ORDER);
        $time = $matches[0];
